...
public class DaoImplMySQL implements Dao {
  ...
  public List<Ticket> findAllTicketsByUserId(Integer userId) {
    ...
    List<Ticket> tickets = jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(Ticket.class), userId);
  ...
  private void enrichTicketWithDetails(Ticket ticket) {
    // 注入 user
    findUserById(ticket.getUserId()).ifPresent(ticket::setUser);

    // 查詢 tran 並注入
    String sql = "select tran_Id, tran_No, date, departureTime, arrivalTime from tran where tran_Id = ?";
    Tran tran = jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(Tran.class), ticket.getTranId());
    ticket.setTran(tran);
  }
